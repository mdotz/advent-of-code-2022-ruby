...
RSpec.describe Day2::Round do
  ...
  context "when accessing player points" do
    let(:calc_double) { instance_double(Day2::RoundPointsCalculator) }

    it "delegates point calculation to RoundPointsCalculator" do
      round = described_class.new(:A, :Z)

      expect(Day2::RoundPointsCalculator).to receive(:new).with(round).and_return(calc_double)
