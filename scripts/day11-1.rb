...
20.times do
  monkeys.each do |monkey|
    monkey.items.each do |item|
      ...
      processed_item = item.send(monkey.operator.to_sym, value.to_i) / 3

      if processed_item % monkey.test_value == 0
        monkeys[monkey.res_true].items << processed_item
      else
        monkeys[monkey.res_false].items << processed_item
      end
